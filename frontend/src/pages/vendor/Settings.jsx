...
import { RiTimeLine, RiStoreLine, RiCheckLine, RiImageAddLine } from 'react-icons/ri';
...
export default function VendorSettings() {
  ...
  const handleSaveHours = async () => {
    ...
    try {
      ...
      const res = await api.put('/vendors/settings', fd, {
        ...
        <div className="p-4 md:p-6 max-w-[600px] mx-auto space-y-4">
          ...
          <div className={S.toggleWrap}>
            ...
            <button
              ...
              disabled={toggling}
              className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer ${vendor?.isOpen ? 'bg-[#00c853]' : 'bg-[#2a2a2a] border border-[#2e2e2e]'}`}>
          ...
          {vendor && (
            <div className={S.cardPaddedLg}>
              ...
              <p className="text-[12px] text-[#555555] mt-4">To update business name or address, contact support.</p>
            </div>
          )}
        </div>
      </main>
...
function InfoRow({ label, value }) {
  return (
    <div className={S.flexBetween + ' py-2 border-b border-[#2e2e2e] last:border-0'}>
      ...
      <span className="text-[14px] font-medium text-[#f0f0f0] capitalize">{value}</span>
    </div>
  );
}